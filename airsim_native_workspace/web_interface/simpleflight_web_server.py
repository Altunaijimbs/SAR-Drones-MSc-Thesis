...
class SimpleFlightWebInterface:
    ...
    def update_state_loop(self):
        """Continuously update drone state"""
        while True:
            try:
                # Get drone state
                ...
                self.drone_state['connected'] = self.client.isApiControlEnabled()
                ...
                # Get camera image
                try:
                    # Try with vehicle_name parameter (newer API)
                    try:
                        responses = self.client.simGetImages([
                            airsim.ImageRequest("0", airsim.ImageType.Scene, False, False)
                        ], vehicle_name="Drone1")
                    except TypeError:
                        # Fallback for older API without vehicle_name
                        responses = self.client.simGetImages([
                            airsim.ImageRequest("0", airsim.ImageType.Scene, False, False)
                        ])
    ...
    def return_home(self):
        def rth_async():
            try:
                ...
                self.client.moveToPositionAsync(home['x'], home['y'], -10, 3).join()
    ...
    def rotate_drone(self, direction):
        def rotate_async():
            try:
                ...
                pos = self.drone_state['position']
                self.client.moveByVelocityZAsync(0, 0, -pos['z'], 1, 
...
def main():
    ...
    print("\n" + "="*60)
    print("SimpleFlight Web Interface")
    print("="*60)
